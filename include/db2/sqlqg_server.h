...
public:
  ...
  // Remote Procedure initialization.
  Procedure* get_procedure (sqluint8*          a_schema_name,
                            sqluint8*          a_proc_name,
                            Procedure_Info*    a_proc_info,
                            Sqlqg_Return_Code* a_rc);
  
  // Drop a specific procedure associated with this server
  void drop_procedure (Procedure* a_proc);

  Sqlqg_Return_Code get_outbound_conn_originator 
                       (sqluint8* a_outbound_orig_userID);
  
  // Drop all procedures associated with this server 
  void drop_all_procedures (void);
...
protected:
  ...
  enum flags_bitmap
  {
    SQLQG_SERVER_UMPLUGIN                    = 0x00000001
  };
 ...
 private:
  ...
  // create_procedure(void) allows a server subclass instance to
  // create an instance of its own remote procedure subclass.
  // The default implementation assumes the wrapper does not support
  // federated stored procedures.
  virtual Procedure* create_procedure (sqluint8*          a_schema_name,
                                       sqluint8*          a_proc_name,
                                       Sqlqg_Return_Code* a_rc);
  ...
  Hash_Table procedures;                // Procedures associated with server 
};
...
inline bool
Server::is_umplugin (void) const
{
  return (m_flags & SQLQG_SERVER_UMPLUGIN);
}

inline Hash_Table*  //@wb055838kaw
Server::get_procedures (void)
{
  return (&procedures);
}                   //@we055838kaw
