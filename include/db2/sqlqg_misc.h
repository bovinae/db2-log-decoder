...
#include "sqlsystm.h"
#include "oss.h" // for OSS_NO_THROW
...
//
// With the clang compiler (or xlC 13.1.2 ppcle), when this no-op is enabled, incorrect calls to new of the form:
//
//    new TYPE(&rc) ;
//    // should be:
//    new(&rc) TYPE(&rc) ; // why some constructors, like Sqlqg_Array::Sqlqg_Array, take an rc is a different question
//
// end up calling the no-op, then blowing up at runtime.  This no-op is dangerous with the clang compiler
// and has been disabled.
//
#if !defined __clang__ && !defined OSS_USING_INTEL16_COMPILER
   #define SQLQG_ENABLE_NOOP_NEW_FOR_PLACEMENT
#endif
...
public:
  ...
  static void *operator new (size_t    a_size,
                             sqlint32* r_rc);
  static inline void *operator new (size_t a_size, void* where){ return where; }
#if defined SQLQG_ENABLE_NOOP_NEW_FOR_PLACEMENT
  static void *operator new (size_t) OSS_NO_THROW ;  // This is a no-op; some compilers
...
public:
  ...
  static void* operator new (size_t     a_size,
                             sqlint32*  a_rc);
  static void* operator new (size_t) OSS_NO_THROW ; // No-op for platform compatibility
  static void operator delete (void*  a_pointer);
...
private:
  ...
  const sqlint16                fixed_key_size;    // key length in bytes
...
public:
  ...
  static void* operator new (size_t     a_size,
                             sqlint32*  a_rc,
                             SQLO_MEM_POOL*  a_pool);
  static void* operator new (size_t) OSS_NO_THROW ; // No-op for compiler compatibility
  static void  operator delete (void* a_pointer);
...
// Sqlqg_Base_Class
inline void*
Sqlqg_Base_Class::operator new (size_t) OSS_NO_THROW
{
  return (void*)NULL;
}
#endif
...
// Hash_Table
inline void*
Hash_Table::operator new (size_t) OSS_NO_THROW
{
  return (void*)NULL;
}
...
// Hash_Entry
inline void*
Hash_Entry::operator new (size_t) OSS_NO_THROW
{
  return (void*)NULL;
};
