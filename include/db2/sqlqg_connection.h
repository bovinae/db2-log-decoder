...
public:
  ...
  // two-phase commit.
  // It is wrapper's responsibility to fill in the sqlca prior to returning any
  ...
  sqluint32 get_outbound_connect_trust_type() const; 
  void set_outbound_connect_trust_type(sqluint32 a_type);
  ...
  virtual sqlint32 create_remote_proc (Runtime_Operation* a_runtime_proc,
                                       Remote_Procedure** a_proc);  
  ...
  void set_user (FencedRemote_User* a_new_user);
  ...
  friend Sint sqlqg_FMP_Insert_Bulk(sqlri_riud* a_runtime_iud, sqlqg_block* block, sqlint32* a_rows);
...
private:
  ...
  // The trust type of the outbound connection can have 1 of 2 possible values:
  // SQLRR_IMPLICIT_TRUSTED_CONNECTION or SQLRR_IMPLICIT_TRUSTED_CONNECTION
  // Please see sqlrr_trusted_contexts.h for the definitions.
  sqluint32 m_outbound_connect_trust_type; 
};
...
inline void
Remote_Connection::set_user (FencedRemote_User* a_new_user)
{
  user = a_new_user;
}
...
inline void 
Remote_Connection::set_outbound_connect_trust_type(sqluint32 a_type)
{
  m_outbound_connect_trust_type = a_type;
}

inline sqluint32
Remote_Connection::get_outbound_connect_trust_type() const
{
  return m_outbound_connect_trust_type;
}
