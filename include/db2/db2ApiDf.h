...
#define db2Version1119   SQL_REL11010900             /* Version 11.1.9.0      */
#define db2Version11500  SQL_REL11050000             /* Version 11.5.0.0      */
#define db2Version11510  SQL_REL11050100             /* Version 11.5.1.0      */
#define db2Version11520  SQL_REL11050200             /* Version 11.5.2.0      */
#define db2Version11530  SQL_REL11050300             /* Version 11.5.3.0      */
#define db2Version11540  SQL_REL11050400             /* Version 11.5.4.0      */
#define db2Version11550  SQL_REL11050500             /* Version 11.5.5.0      */
...
#define DB2REORG_RECLAIM_EXTENTS  10      /* Define for reorg table reclaim   */
                                          /* extents                          */
#define DB2REORG_RECOMPRESS       11      /* Define for reorg table           */
                                          /* recompress                       */
...
/******************************************************************************
** db2Credentials data structure.  Used for specifying various types of credentials
** required for connections or attachments.  Not all parameters should be used
** at the same time, only those required.  All other values should be NULL.
** 
** db2Credentials data structure parameters
** 
** pUsername
** Input. A string containing the username. Can be NULL.
** 
** pPassword
** Input. A string containing the password. Can be NULL.
** 
** pNewPassword
** Input. A string containing the new password. Can be NULL. Not all interfaces
** using the db2Credentials structure may support changing passwords.
** 
** pAccessToken
** Input. A string containing the access token. Can be NULL.
** 
** pAccessTokenType
** Input. A string containing the access token type. Can be NULL.
** 
** pApiKey
** Input. A string containing the API key. Can be NULL.
** 
** pDelegatedCredentialHandle
** Input. A pointer to the GSSAPI delegated credential. Can be NULL.
** 
** pReserved1
** Input. Reserved for future use. Must be NULL.
** 
** pReserved2
** Input. Reserved for future use. Must be NULL.
** 
...
** 
*******************************************************************************/
typedef SQL_STRUCTURE db2Credentials
{
...
/******************************************************************************
** db2InstanceAttachStruct data structure parameters
** 
** piInstanceName
** Input. A string containing the alias of the instance to which
** the user wants to attach. This instance must have a matching entry in the
** local node directory. The only exception is the local instance (as specified
** by the DB2INSTANCE environment variable), which can be specified as the object
** of an attachment, but cannot be used as a node name in the node directory. Can be NULL.
** 
** pCredentials
** Input. A pointer to the db2Credentials structure containing the credentials to
** be used for the attach.  May be NULL if no credentials are used.
** 
*******************************************************************************/
...
** (CREATE DATABASE and FORCE APPLICATION, for example) are to be executed.
** 
** This node may be the current instance (as defined by the value of the DB2INSTANCE
** environment variable), another instance on the same workstation, or an instance
** on a remote workstation. Establishes a logical instance attachment to the node specified,
** and starts a physical communications connection to the node if one does not already exist.
** 
** Authorization
** 
** None
...
** server.
** 
** If the instance name is a zero-length string or NULL, information about
...
** 
** If an attachment exists, and the API is issued with an instance name, the current
** attachment is dropped, and an attachment to the new instance is attempted.
** 
** Where the user name and password are authenticated depends on the
** authentication type of the target instance.
** 
** The instance to which an attachment is to be made can also be specified by a call
** to the sqlesetc API.
** 
** Specifying a new password in the db2Credentials struct will attempt to change
** the user's password.  The Db2 database system provides support for changing passwords
